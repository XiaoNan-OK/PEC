...
def compute_tqg_matrices(sampler, input_circuit: QuantumCircuit, shots:int=1024, obs_batch:int=256):
    active=_used_qubit_indices(input_circuit)
    m=len(active)
    mats={}
    row_labels=col_labels=None
    for key in unique_cnot_keys(input_circuit):
        M,rlabs,clabs=compute_matrix_for_tqg(sampler, input_circuit, key, shots, obs_batch)
        mats[key]=M
