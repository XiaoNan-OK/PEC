...
def _sanitize_for_pickle(obj):
    if isinstance(obj, dict):
        ...
        for key, value in obj.items():
            if key == "jobs":
                sanitized["job_ids"] = [
                    job.job_id() for job in value if hasattr(job, "job_id")
                ]
                continue
            sanitized[key] = value if _is_pickleable(value) else _sanitize_for_pickle(value)
        return sanitized
    if isinstance(obj, list):
        out = []
        for item in obj:
            if _is_pickleable(item):
                out.append(item)
            elif hasattr(item, "job_id"):
                out.append({"job_id": item.job_id()})
            else:
                out.append(_sanitize_for_pickle(item))
        return out
    if isinstance(obj, tuple):
        return tuple(_sanitize_for_pickle(item) for item in obj)
...
def save_readout_result(result, 
    ...
    payload = _sanitize_for_pickle(result)
