...
from mitigation.PEC_Sampling import run_tqg_pec_package_sampler
...
def _bind_sampler_to_backend(sampler, backend):
    if backend is None:
        return sampler
...
def _run_sampler_job(sampler, circuits, shots):
    return sampler.run(circuits, shots=shots)

def build_meas_vector_nopec_sampler(sampler, base: QuantumCircuit, backend, *, opt_level=0, shots=4096, meas_qubits=(0,1), return_jobs: bool = False):
    """
    ...
    sampler = _bind_sampler_to_backend(sampler, backend)
    ...
    primitive_results = []
    if len(circs) > 0:
        job = _run_sampler_job(sampler, circs, shots)
        jobs.append(job)
        res = job.result()
    ...
    print(m)
    if return_jobs:
        return m, jobs, primitive_results
    return m
...
def one_trial_nopec_S_sampler(
    ...
    *,
    ...
    """
    circ = base_circuit
    nopec_output = build_meas_vector_nopec_sampler(
        sampler,
        circ,
        backend,
        opt_level=opt_level,
...
def one_trial_pec_S_sampler(
    ...
    *,
    ...
    if return_jobs:
        return float(cs["S"]), cs, results, results.get("_jobs", []), results.get("_primitive_results", [])
...
def compare_pec_vs_nopec_sampler(
    ...
    *,
    ...
    for i in range(1, n_trials+1):
        nopec_output = one_trial_nopec_S_sampler(
            sampler, qcircuit, backend=backend, opt_level=opt_level, shots=shots,
            meas_qubits=meas_qubits, return_jobs=return_jobs
        )
        if return_jobs:
            s_nopec, cs_nopec, meas_nopec, jobs_nopec, _ = nopec_output
            nopec_jobs.append(jobs_nopec)
        else:
            s_nopec, cs_nopec, meas_nopec = nopec_output
        pec_output = one_trial_pec_S_sampler(
        ...
        if return_jobs:
            s_pec, cs_pec, meas_pec, jobs_pec, _ = pec_output
            pec_jobs.append(jobs_pec)
        else:
            s_pec, cs_pec, meas_pec = pec_output
    ...
    if return_jobs:
        summary["PEC_job_ids"] = [[job.job_id() for job in batch if hasattr(job, "job_id")] for batch in pec_jobs]
        summary["noPEC_job_ids"] = [[job.job_id() for job in batch if hasattr(job, "job_id")] for batch in nopec_jobs]
