...
def estimate_over_all_observables(estimator, circuit: QuantumCircuit, obs_labels: List[str], 
    ...
    for k0 in range(0, len(obs_labels), obs_batch):
        ...
        pub = (circuit, observables, None)   # (circuit, [O1, O2, ...], params=None)
        res = estimator.run([pub]).result()
        evs = res[0].data.evs
        out[k0:k1] = np.asarray(evs, dtype=float).reshape(-1)
        # vals = []
        # for i in range(len(batch)):
        #     evs = np.asarray(res[i].data.evs).ravel()
        #     vals.append(float(evs[0]))
        # out[k0:k1] = np.asarray(vals, dtype=float)
    return out 
...
def tqg_pec_single_circuit_joint(estimator,
    ...
    if m == 0:
        ...
        if t_backend is not None:
            cir = transpile(cir, backend=t_backend, optimization_level=t_opt_level)
        pubs = [(cir, [observable], None)]
    ...
    while True:
        ...
        for idx_tuple in batch_idx_tuples:
            # Build label list for all CNOTs and the product of their weights
            ...
            if t_backend is not None:
                tw = transpile(tw, backend=t_backend, optimization_level=t_opt_level)
        ...
        # Accumulate sum of (weight product) * expectation value
        for j in range(len(circs)):
            ev = float(np.asarray(res[j].data.evs).ravel()[0])
            acc += weight_batch[j] * ev
